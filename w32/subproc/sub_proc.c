...
find_file(const char *exec_path, const char *path_var,
  ...
  strcpy_s(fname, strlen(exec_path) + 5, exec_path);
  ...
  */
  if(exec_handle == INVALID_HANDLE_VALUE) {
    if(exec_path[0] == '/') {
      ...
      char **argvi = argv;
      int arglen = 0;
      strcpy_s(buf, sizeof(buf), variable_expand("$(SHELL)"));
      shell_name = &buf[0];
      strcpy_s(exec_fname, sizeof(exec_fname), "-c");
      ...
      for(argvi = argv; *argvi; argvi++) {
        strcat_s(new_argv0, (arglen + 1), *argvi);
        strcat_s(new_argv0, (arglen + 1), " ");
      }
