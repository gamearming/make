...
same_stream(FILE *f1, FILE *f2) {
  HANDLE fh1 = (HANDLE)_get_osfhandle(_fileno(f1));
  HANDLE fh2 = (HANDLE)_get_osfhandle(_fileno(f2));
...
tmpfile(void) {
  ...
  while(path_size > 0 &&
    ...
    HANDLE h;
    sprintf_s(temp_path + path_size, sizeof(temp_path) - path_size,
              "%s%s%u-%x.tmp",
...
dlopen(const char *file, int mode) {
  ...
  if(!file)
    ...
    strcpy_s(dllfn, sizeof(dllfn), file);
    for(p = dllfn; *p; p++)
      if(*p == '/')
        *p = '\\';
...
dlerror(void) {
  ...
  errbuf[ret] = '\0';
  if(!ret)
    sprintf_s(errbuf, sizeof(errbuf), "Error code %lu", last_err);
  last_err = 0;
  return errbuf;
}
#pragma warning( push )
