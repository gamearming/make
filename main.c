...
decode_output_sync_flags(void) {
  ...
  if(output_sync_option) {
    if(streq(output_sync_option, (char*) "none"))
      output_sync = OUTPUT_SYNC_NONE;
    else if(streq(output_sync_option, (char*) "line"))
      output_sync = OUTPUT_SYNC_LINE;
    else if(streq(output_sync_option, (char*) "target"))
      output_sync = OUTPUT_SYNC_TARGET;
    else if(streq(output_sync_option, (char*) "recurse"))
...
#endif
{
  ...
  /* Request the most powerful version of 'system', to make up for the dumb default shell.  */
  ...
  // 強制轉換為 void 關閉在停用 NLS 的系統上的編譯器警告。
  ...
  if(argv[0][0] == '\0')
    ...
    if(program == 0) {
      ...
      if(program) {
        ...
        if(argv0_len > 4 && streq(&program[argv0_len - 4], (char*) ".exe"))
