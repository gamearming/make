...
eval_makefile(const char *filename, int flags) {
  ...
  deps->error = (unsigned short) errno;
  ...
  deps->flags = (unsigned short) flags;
  free(expanded);
  if(ebuf.fp == 0) {
...
#define record_waiting_files()                                                \
  ...
  while(1) {
    ...
    if(ebuf->floc.lineno == 1 && line[0] == 0xEF && line[1] == 0xBB && line[2] == 0xBF) {
      line += 3;
      if(ISDB(DB_BASIC)) {
        if(ebuf->floc.filenm)
...
record_files(struct nameseq *filenames, const char *pattern,
  ...
  while(1) {
    ...
    } else if(streq(name, (char*) ".SECONDEXPANSION"))
      second_expansion = 1;
    #if !defined (__MSDOS__) && !defined (__EMX__)
    else if(streq(name, (char*) ".ONESHELL"))
      one_shell = 1;
    #endif
