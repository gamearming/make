...
func_lastword(char *o, char **argv, const char *funcname UNUSED) {
  ...
  const char *p = NULL;
  const char *t;
  while(NULL != (t = find_next_token(&words, &i))) {
    p = t;
  }
  if(p != 0)
    o = variable_buffer_output(o, p, i);
  ...
  *stringp = end;
  ...
  if(entry_p->expand_args) {
    ...
    for(p = beg, nargs = 0; p <= end; ++argvp) {
      const char *next;
      ++nargs;
      //if(nargs == entry_p->maximum_args || (!(next = find_next_argument(openparen, closeparen, p, end))))
      if(nargs == entry_p->maximum_args || NULL == (next = find_next_argument(openparen, closeparen, p, end)))
        next = end;
    ...
    for(p = abeg, nargs = 0; p <= aend; ++argvp) {
      char *next;
      ++nargs;
      //if(nargs == entry_p->maximum_args || (!(next = find_next_argument(openparen, closeparen, p, aend))))
      if(nargs == entry_p->maximum_args || NULL == (next = find_next_argument(openparen, closeparen, p, aend)))
        next = aend;
      *argvp = p;
      *next = '\0';
      p = next + 1;
