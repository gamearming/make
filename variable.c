...
lookup_special_var(struct variable *var) {
  ...
  */
  if(variable_changenum != last_changenum && streq(var->name, (char*) ".VARIABLES")) {
      ...
      if(! HASH_VACANT(*v_slot)) {
        ...
        switch(v->export) {
          ...
          case v_noexport: {
              ...
              if(streq(v->name, (char*) "SHELL") && shell_var.value) {
                v = &shell_var;
                break;
              }
              continue;
...
set_special_var(struct variable *var) {
  if(streq(var->name, (char*) RECIPEPREFIX_NAME)) {
